...
nets = dict(
    ...
    ircoin=math.Object(
        P2P_PREFIX='06060708'.decode('hex'),
        P2P_PORT=57288,
        ADDRESS_VERSION=102,
        RPC_PORT=57284,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'ircoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 65536*100000000 >> (height + 1)//50000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=60, # s
        SYMBOL='IRC',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'IRCoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/IRCoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.IRCoin'), 'ircoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://irc-explorer.zenet.org/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://irc-explorer.zenet.org/address/',
        TX_EXPLORER_URL_PREFIX='http://irc-explorer.zenet.org/tx/',
        SANE_TARGET_RANGE=(2**256//100000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.03e8,
    ),
